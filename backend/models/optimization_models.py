...
class OptimizationRequest(BaseModel):
    ...
    max_credits: float = Field(
        ..., gt=0, le=10, description="Maximum credit units allowed"
    )
    min_credits: float = Field(
        ..., ge=0, description="Minimum utility threshold for courses"
    )
    seed: int = Field(..., ge=0, description="Random seed for price calculation")
    courses: List[CourseInput] = Field(
        ..., min_length=1, description="List of courses with utilities"
    )
...
class OptimizedCourse(BaseModel):
    ...
    selected: bool = Field(
        ..., description="Whether course was selected in optimization"
    )
...
class OptimizationResponse(BaseModel):
    ...
    selected_courses: List[OptimizedCourse] = Field(
        ..., description="Courses selected by optimization"
    )
    ...
    optimization_status: str = Field(
        ..., description="Status of optimization (Optimal, Infeasible, etc.)"
    )
    message: Optional[str] = Field(
        default=None, description="Additional information about the optimization"
    )
