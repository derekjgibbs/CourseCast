            ...
            try:
                ...
                if 'solver_results' in st.session_state:
                    ...
                    st.session_state.prev_metrics = {
                        ...
                        'weighted_utility': (prev_selected['Utility'] * prev_selected['credit_unit']).sum()
                    }
                
                # Store new results
                st.session_state.solver_results = selected
                st.session_state.selected_uniqueids = [item['uniqueid'] for item in selected]
                
                st.sidebar.success("✨ Optimization complete! Click 'Schedule Forecast' tab to view your schedule.")
    ...
    if st.sidebar.button("Simulate Schedule (100x)", type="primary"):
        ...
        if len(courses_with_utility) == 0:
            ...
            try:
                ...
                st.sidebar.success("🎲 Simulation complete! Click 'Schedule Simulation' tab to view the analysis.")
...
tab1, tab2, tab3 = st.tabs(["Course Browser", "Schedule Forecast", "Schedule Simulation"])
...
with tab1:
    ...
    # Remove the toggle and always show evaluation filters
    ...
    # Modify the display columns to always include evaluation columns
    display_columns = [
        'Utility', 
        'primary_section_id', 
        'title', 
    ...
    # Evaluation filters (now always active)
    ...
    # Create a container for the buttons and status
    save_container = st.container()

    # Create a container for the dataframe
    df_container = st.container()

    # Add buttons side by side in the first container
    with save_container:
        col1, col2 = st.columns([1, 4.8])
        
        with col1:
            if st.button("Save Utility Values"):
                if 'utility_data' in st.session_state and filtered_data is not None:
                    utility_updates = filtered_data.set_index('primary_section_id')['Utility']
        ...
        with col2:
            with open("data_spring_2025.xlsx", "rb") as file:
                btn = st.download_button(
                    label="📥 Download Course Dataset (Excel)",
                    data=file,
                    file_name="data_spring_2025.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    help="Download the complete dataset including all courses and evaluations"
                )

    # Display the dataframe in the second container
    with df_container:
...
with tab2:
    ...
    if 'selected_uniqueids' not in st.session_state or not st.session_state.selected_uniqueids:
        st.info("Click 'Forecast Schedule (1x)' in the sidebar to see optimization results here.")
    else:
        # First, create a dictionary of uniqueid -> price from solver results
        solver_prices = {item['uniqueid']: item['price'] for item in st.session_state.solver_results}
        ...
        if 'prev_metrics' in st.session_state:
            delta_credits = selected_courses['credit_unit'].sum() - st.session_state.prev_metrics['credits']
            delta_price = selected_courses['price_predicted'].sum() - st.session_state.prev_metrics['price']
            weighted_utility = (selected_courses['Utility'] * selected_courses['credit_unit']).sum()
            delta_utility = weighted_utility - st.session_state.prev_metrics['weighted_utility']
        
        with summary_cols[0]:
            st.metric(
                "Total Credits", 
                f"{selected_courses['credit_unit'].sum():.1f}",
                delta=f"{delta_credits:.1f}" if delta_credits is not None and delta_credits != 0 else None
            )
        with summary_cols[1]:
            st.metric(
                "Total Price", 
                f"{selected_courses['price_predicted'].sum():,.0f}",
                delta=f"{delta_price:,.0f}" if delta_price is not None and delta_price != 0 else None
            )
        with summary_cols[2]:
            weighted_utility = (selected_courses['Utility'] * selected_courses['credit_unit']).sum()
            st.metric(
                ...
                delta=f"{delta_utility:,.0f}" if delta_utility is not None and delta_utility != 0 else None
            )
    ...
    if 'monte_carlo_results' not in st.session_state:
        ...
        course_probs = sorted(
            [
                ...
                for course_id, prob in st.session_state.monte_carlo_results['course_probabilities'].items()
                for course_info in [st.session_state.utility_data[st.session_state.utility_data['uniqueid'] == course_id].iloc[0]]
            ],
        ...
        for i, schedule in enumerate(st.session_state.monte_carlo_results['schedule_probabilities'][:3], 1):
            ...
            weighted_utility = (schedule_courses['Utility'] * schedule_courses['credit_unit']).sum()
