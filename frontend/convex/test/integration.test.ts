...
describe("Integration tests - User and Scenario workflows", () => {
  ...
  describe("Error handling workflows", () => {
    ...
    test("handles constraint violations gracefully", () => {
      ...
      expect(invalidScenarioData.max_credits).toBeGreaterThan(CONSTRAINTS.USER_SCENARIO.MAX_CREDITS_LIMIT);
      expect(invalidScenarioData.min_credits).toBeLessThan(CONSTRAINTS.USER_SCENARIO.MIN_CREDITS_LIMIT);
      expect(Object.values(invalidScenarioData.utilities)[0]).toBeGreaterThan(CONSTRAINTS.COURSE_UTILITY.MAX_VALUE);
  ...
  describe("Complex scenario workflows", () => {
    ...
    test("scenario sharing and privacy", () => {
      ...
      const user1Scenarios = [
        createMockUserScenario(user1Id),
        createMockUserScenario(user1Id),
      ];
      
      const user2Scenarios = [
        createMockUserScenario(user2Id),
      ];
