...
class Settings(BaseSettings):
    ...
    convex_auth_token: Optional[str] = Field(
        default=None, description="Convex auth token"
    )
    ...
    max_monte_carlo_iterations: int = Field(
        default=1000, description="Maximum Monte Carlo iterations"
    )
    optimization_timeout: int = Field(
        default=300, description="Optimization timeout in seconds"
    )
