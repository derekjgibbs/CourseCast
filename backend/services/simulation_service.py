...
from models.simulation_models import SimulationRequest, SimulationResponse, CourseStatistics, ScheduleStatistics
...
class SimulationService:
    ...
    def run_simulation(self, request: SimulationRequest) -> SimulationResponse:
        ...
        course_probabilities = self._calculate_course_probabilities(request, simulation_results, successful_simulations)
        schedule_probabilities = self._calculate_schedule_probabilities(schedule_counter, successful_simulations)
    ...
    def _execute_simulations(self, request: SimulationRequest):
        ...
        for i in range(request.num_simulations):
            try:
                ...
                selected_courses = [course for course in result.selected_courses if course.selected]
    ...
    def _calculate_course_probabilities(self, request: SimulationRequest, simulation_results, successful_simulations):
        ...
        return [
            CourseStatistics(
                uniqueid=uniqueid,
                probability=count / successful_simulations if successful_simulations > 0 else 0,
                selection_count=count
            )
            for uniqueid, count in course_counts.items()
        ]
    
    def _calculate_schedule_probabilities(self, schedule_counter, successful_simulations):
